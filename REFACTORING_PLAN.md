...
> **進捗（1stスライス済み）**: esbuild バンドル方式を確立（`src/main.js` → `app.js`）。
> `function-calling.js` / `dropbox.js` / `app.js` を `src/` 配下の ESM モジュールへ移送し、
> 1つの `app.js` バンドルに統合。クロスファイルの bare グローバル参照を `window.*` へ統一。
> 純粋ユーティリティ（`utils/format.js` / `utils/html.js`）を抽出してユニットテストを追加。
> ビルド手順は `docs/build.md` を参照。
> **残作業**: `src/app.js` 内の神オブジェクト（`dbUtils`/`uiUtils`/`apiUtils`/`appLogic`）と
> 定数群をモジュールへ切り出し、`window.*` 経由の暫定結合を `import` へ置換していく。
