...
## Phase 1 — モジュール化の地ならし（ESM 化） 🚧 着手中

> **進捗（モジュール分割まで完了、実機QA待ち）**:
> - esbuild バンドル方式を確立（`src/main.js` → `app.js`）。`function-calling.js` /
>   `dropbox.js` / `app.js` を `src/` の ESM へ移送し1バンドルに統合。
> - 純粋ユーティリティ（`utils/format.js` / `utils/html.js`）を抽出しユニットテスト追加。
> - 定数・state・DebugLogger・MIME表をモジュール化（`constants.js` / `state.js` /
>   `debug-logger.js` / `mime-types.js`）。
> - **神オブジェクトを分割**: `db.js`(dbUtils) / `ui.js`(uiUtils) / `api.js`(apiUtils) /
>   `app-logic.js`(appLogic) / `dom-elements.js`(elements) へ切り出し。`src/app.js` は
>   14,638行 → 約1,520行のオーケストレータに縮小。モジュール間は ESM 循環 import で連携。
> - 検証: 抽出前後でメソッド数一致（appLogic 115 / uiUtils 56 / dbUtils 21 / apiUtils 12）、
>   バンドルサイズ・window公開不変、lint ベースライン維持（errors 13）、test グリーン。
> - ビルド手順・構成は `docs/build.md` を参照。
>
> **残作業**: `window.*` 経由の暫定結合（`function-calling.js` / `dropbox.js`）の整理は
> Phase 2 以降で対応。`app-logic.js`（8,360行）の機能別分割は Phase 3 で実施。
