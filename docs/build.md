...
src/
  ...
  app.js               オーケストレータ（初期化・window公開・末尾IIFE・グルー関数）
  constants.js         全定数（モデル一覧・APIエンドポイント・各種閾値など）
  state.js             実行時状態オブジェクト state
  dom-elements.js      DOM要素レジストリ elements（ロード時に querySelector で構築）
  debug-logger.js      DebugLogger（state に依存）
  mime-types.js        拡張子→MIME 対応表
  db.js                dbUtils（IndexedDB 操作）
  ui.js                uiUtils（DOM 描画・UI 制御）
  api.js               apiUtils（各プロバイダー API 通信）
  app-logic.js         appLogic（アプリ本体ロジック。Phase 3 で機能別に更に分割予定）
  function-calling.js  Function Calling 定義
  dropbox.js           Dropbox 連携
  utils/
...
各モジュールは ESM の `import`/`export` で連携する。神オブジェクト（dbUtils /
uiUtils / apiUtils / appLogic）は相互参照するが、参照はすべて**メソッド呼び出し時**に
解決されるため、ESM の循環 import でも安全に動作する（モジュール本体はオブジェクト
リテラルの定義のみで、ロード時の副作用がない）。

appLogic が使う一部のグルー関数（`broadcastChannel` / `setupBroadcastChannel` /
`registerServiceWorker` / `updateMessageMaxWidthVar`）は app.js に残し、`export` して
app-logic.js から import している。`function-calling.js` / `dropbox.js` は従来どおり
`window.*`（`window.appLogic` 等）経由で連携する。
