...
| 設定 | 内容 |
|---|---|
| **読み上げスタイル**（プルダウン） | 登録したプリセットから選ぶ。声そのものは音声IDのまま |
| **スタイルの登録・編集** | 「スタイル名」と「指示」を入力して保存すると、名前がプルダウンに並びます |
| **「指定なし」のときに使う指示** | プリセット未選択のときに使われる指示（例：`明るく元気で、楽しそうな話し方。`） |
| **話す速さ** | 0.25〜4.0（既定1.0） |
| **声の寄せ具合** | 参照音声への似せ具合。「あまり似ていない」と感じたら上げる |
| **選択した範囲だけ読み上げる** | ONのとき、メッセージ内で文字を選択してからボタンを押すとその部分だけ再生します（音声保存も同様）。選択が無ければ全文を読むので、ONのままでも普段の使い方は変わりません |

### スタイルのプリセット

設定の「スタイルの登録・編集」で、**スタイル名**と**指示**を入力して「このスタイルを保存」を押します。

| スタイル名 | 指示 |
|---|---|
| 通常 | 落ち着いた自然な話し方。 |
| 怒りモード | 強い怒りを込めた、荒く低い口調。 |
| ASMR風 | 耳元でささやくような、ごく静かで甘い声。 |

登録した名前がプルダウンに並ぶので、選ぶだけで話し方を切り替えられます。

- プルダウンでスタイルを選ぶと、その内容が登録フォームに読み込まれて編集できます
- 同じ名前で保存すると**上書き**、新しい名前なら**追加**されます
- 「削除」を押すと、スタイル名欄に入っている名前のプリセットを消します
- プリセットを選んでいるときは、そちらが「指定なし」用の指示より優先されます
- プリセットを削除した場合、選択は自動的に「指定なし」へ戻ります
