...
| **スタイルのプリセット** | 1行に1件「名前,指示」で登録。名前がプルダウンに並びます |
| **スタイルの自由入力** | プリセット未選択のときに使われる指示（例：`明るく元気で、楽しそうな話し方。`） |
| **話す速さ** | 0.25〜4.0（既定1.0） |
...
設定の「スタイルのプリセット」に、1行ずつ「名前,指示」の形式で登録します：
...
- 区切りは**最初の**「,」「、」「:」「：」です。指示の中の「、」はそのまま使えます
- プリセットを選んでいるときは、そちらが自由入力欄より優先されます
- プリセットの定義を消した場合、選択は自動的に「指定なし」へ戻ります
